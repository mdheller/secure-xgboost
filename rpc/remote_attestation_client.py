...
def run(channel_addr, train_fname, train_key, test_fname, test_sym_key):
    """
    ...
    # This keypair is the public/private key of the client
    keypair = "keypair.pem"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ip-addr", help="server IP address")
    parser.add_argument("--train-fname", help="path to training data file on server")
    parser.add_argument("--train-key", help="path to key used to encrypt training data on client")
    parser.add_argument("--test-fname", help="path to test data file on server")
    parser.add_argument("--test-key", help="path to key used to encrypt test data on client")

    channel_addr = str(args.ip_addr) + ":50051" 

    logging.basicConfig()
    run(channel_addr, str(args.train_fname), str(args.train_key), str(args.test_fname), str(args.test_key))
